...
from vllm.core.block.interfaces import CompoundBlock, BlockInfo
...
class BlockSpaceManagerV2(BlockSpaceManager):
    ...
    def can_swap_in(self, seq_group: SequenceGroup,
        ...
        return self._can_swap(seq_group, Device.GPU, SequenceStatus.SWAPPED,
                              num_lookahead_slots)
    def _update_block_table(self, seq: Sequence):
        pass
    def swap_in(self, seq_group: SequenceGroup) -> List[Tuple[int, int]]:
        ...
        left, right, new_block = self.block_allocator.swap(
            blocks=blocks, source_device=Device.CPU, dest_device=Device.GPU)

        return self.convert_list_swap_info_to_mapping(left, right) # type: ignore
    ...
    def convert_list_swap_info_to_mapping(self, left: List[BlockInfo], right: List[BlockInfo]) -> List[Tuple[int, int]]:
        left_list = []
        right_list = []
        for l in left:
            left_list.extend(l.to_list())
        for r in right:
            right_list.extend(r.to_list())
        return zip(left_list, right_list) # type: ignore
    ...
    def swap_out(self, sequence_group: SequenceGroup) -> List[Tuple[int, int]]:
        ...
        blocks = self._get_blocks_for_swap(sequence_group,
                                           SequenceStatus.RUNNING)
        left, right = self.block_allocator.swap(
    ...
    def _get_blocks_for_swap(self, seq_group: SequenceGroup,
                             status: SequenceStatus) -> List[CompoundBlock]:
        ...
        """
        blocks: Dict[int, List[CompoundBlock]] = {}
